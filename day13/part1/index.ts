...
function solve(machine: Machine) {
  ...
  solver.addConstraint(constraint1, "EQ", machine.prize.x);
  solver.addConstraint(constraint2, "EQ", machine.prize.y);
  ...
  const res = feasable ? Math.round(solver.getObjectiveValue()) : 0;
  return res;
}
